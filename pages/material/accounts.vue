	...
	<view> <cu-custom :bgColor="NavBarColor" :isBack="true" backRouterName="material">
		...
		<view class="search-box">
			<view class="input-row">
				...
				<uni-icons type="search" @tap="openAccountSearch" size="25"></uni-icons>
			</view>
		</view>
		<uni-popup ref="popup" style="z-index: 20; margin: 0 auto;height: 800px;" type="right">
			<view style="height: 800px; width: 100%;float: left;">
				<form style="height: 800px;height: 800px;">
					...
					<view class="cu-form-group">
						...
						<view>
							<input prefix-icon="search" placeholder="请输入编号查询" v-model="serialNo" />
						</view>
					</view>
					<view class="cu-form-group">
		...
		<view class="stock-list" style="[{animation: 'show 0.5s 1'}]">
			<mescroll-uni ref="mescrollRef" @init="mescrollInit" :top="top" @down="downCallback" @up="upCallback">
				<view
					...
					style="width: 100vw;" v-for="(account,index) in accounts" :key="index"
					@tap="goaccountsDetail(account)" :class="modalName=='move-box-'+ index?'move-cur':''"
					...
					<view class="stock-item" style="min-height: 60px;width: 100vw">
						...
						<view class="item-row">
							...
							<text class="value">{{ account.initialAmount || '-' }}</text>
		...
		methods: {
			...
			loadinfo() {
				this.$http.get(this.accountsUrl, {
					params: {
						search: '{"name":"' + this.name + '","serialNo":"' + this.serialNo + '","remark":"' + this
							.remark + '"}',
					}
				}).then(res => {
			...
			upCallback(page) {
				...
				params.search = '{"name":"' + this.name + '","serialNo":"' + this.serialNo + '","remark":"' + this.remark +
					'"}',
					//联网加载数据
					this.$http.get(this.accountsUrl, {
						params: params
						...
						if (page.num == 1) {
							this.accounts = []; //如果是第一页需手动制空列表
						}
						if (res.data.code == "200") {
							this.accounts = this.accounts.concat(res.data.data.rows); //追加新数据
