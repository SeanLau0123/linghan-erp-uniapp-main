...
<template>
	<view> <cu-custom :bgColor="NavBarColor" :isBack="true" backRouterName="material">
			...
			<block slot="content">多属性</block>
		</cu-custom>
		<view class="search-box">
			<view class="input-row">
				<input class="search-input" placeholder="请输入属性名" v-model="search" @confirm="confirm" />
				<uni-icons type="scan" size="25" @tap="showScan"></uni-icons>
				<uni-icons type="search" @tap="openattributeItemearch" size="25" style='margin-right: 5px;'></uni-icons>
			</view>
		</view>
			...
			<view style="height: 800px; width: 100%;float: right;">
				<form style="height: 800px;">
					<view class="cu-form-group" style="margin-top:60px;">
						...
						<input prefix-icon="search" placeholder="请输入属性名" v-model="search" />
					</view>
		...
		<view class="stock-list" style="[{animation: 'show 0.5s 1'}]">
			<mescroll-uni ref="mescrollRef" @init="mescrollInit" :top="top" @down="downCallback" @up="upCallback">
				<view
					...
					style="width: 100vw;" v-for="(attribute,index) in attributeItem" :key="index"
					@tap="goattributeItemDetail(attribute)" :class="modalName=='move-box-'+ index?'move-cur':''"
					:data-target="'move-box-' + index">
					<view class="stock-item" style="min-height: 20px;width: 100vw">
						<view class="item-row">
							<text class="label">属性名：</text>
							<text class="value name">{{ attribute.attributeName || '-' }}</text>
						</view>
						<view class="item-row">
							<text class="label">属性值：</text>
							<text class="value">{{ attribute.attributeValue || '-' }}</text>
						</view>
					</view>
				</view>
			</mescroll-uni>
	...
	export default {
		...
		data() {
			return {
				search: '',
				attributeItem: [],
				attributeItemUrl: '/materialAttribute/list',
				modalName: null,
				read: "all",
				msg1Count: "",
				msg1Title: ""
		...
		methods: {
			...
			loadinfo() {
				this.$http.get(this.attributeItemUrl, {
					params: {
						search: '{"attributeName":"' + this.search + '"}',
					}
				}).then(res => {
					if (res.data.code == "200" && res.data.data.rows.length != 0) {
						let result = res.data.data;
						this.attributeItem = result.rows;
					} else {
			...
			handleSearch() {
				if (!this.searchText.trim()) {
					this.filteredattributeItem = [...this.attributeItem];
					return;
				}
				const keyword = this.searchText.toLowerCase();
				this.filteredattributeItem = this.attributeItem.filter(account =>
			...
			upCallback(page) {
				...
				params.search = '{"attributeName":"' + this.search + '"}',
					//联网加载数据
					this.$http.get(this.attributeItemUrl, {
						params: params
					}).then(res => {
						if (page.num == 1) {
							this.attributeItem = []; //如果是第一页需手动制空列表
						}
						if (res.data.code == "200") {
							this.attributeItem = this.attributeItem.concat(res.data.data.rows); //追加新数据
			...
			goattributeItemDetail(account) {
				this.mescroll.resetUpScroll()
				uni.navigateTo({
					url: '/pages/material/attributeItem/attributeItemInfo?attributeItem=' + encodeURIComponent(JSON
						.stringify(
							account)) + '&action=edit'
				});
			},
			fabClick() {
				uni.navigateTo({
					url: '/pages/material/attributeItem/attributeItemInfo?action=add'
				});
			},
			ListTouchStart(e) {
