...
<template>
	<view> <cu-custom :bgColor="NavBarColor" :isBack="true" backRouterName="material">
			...
			<block slot="content">经办人管理</block>
		</cu-custom>
		<view class="search-box">
			<view class="input-row">
				...
				<uni-icons type="search" @tap="openpersonsearch" size="25" style='margin-right: 5px;'></uni-icons>
			</view>
		</view>
			...
			<view style="height: 800px; width: 100%;float: right;">
				<form style="height: 800px;">
							...
							<view class="cu-form-group">
								...
								<view>
									<app-select v-model="personType" placeholder="请选择类型" :dict="personList"
										space></app-select>
								</view>
							</view>
		...
		<view class="stock-list" style="[{animation: 'show 0.5s 1'}]">
			<mescroll-uni ref="mescrollRef" @init="mescrollInit" :top="top" @down="downCallback" @up="upCallback">
				<view
					...
					style="width: 100vw;" v-for="(item,index) in persons" :key="index" :class="modalName=='move-box-'+ index?'move-cur':''"
					...
					<view class="item-row">
						...
						<view style="margin-right: 20rpx; font-weight: bold;">
							<text v-if="item.enabled == '0'" style="color:red;">禁用</text>
							<text v-if="item.enabled == '1'" style="color:green;">启用</text>
						</view>
					</view>
					<view class="item-row">
		...
		methods: {
			...
			loadinfo() {
				this.$http.get(this.personsUrl, {
					params: {
						search: '{"name":"' + this.name + '","type":"'+this.personType+'"}',
					}
				}).then(res => {
			...
			upCallback(page) {
				...
				params.search= '{"name":"' + this.name + '","type":"'+this.personType+'"}',
					//联网加载数据
					this.$http.get(this.personsUrl, {
						params: params
					}).then(res => {
						if (page.num == 1) {
							this.persons = []; //如果是第一页需手动制空列表
						}
						if (res.data.code == "200") {
							this.persons = this.persons.concat(res.data.data.rows); //追加新数据
