	...
	<view> <cu-custom :bgColor="NavBarColor" :isBack="true" backRouterName="material">
		...
		<view class="search-box">
			<view class="input-row">
				<input class="search-input" placeholder="请输入角色名称" v-model="name" @confirm="confirm" />
				<uni-icons type="scan" size="25" @tap="showScan"></uni-icons>
				<uni-icons type="search" @tap="openroleSearch" size="25" style='margin-right: 5px;'></uni-icons>
			</view>
		</view>
			...
			<view style="height: 800px; width: 100%;float: right;">
				<form style="height: 800px;">
					<view class="cu-form-group" style="margin-top:60px;">
						...
						<input prefix-icon="search" placeholder="请输入角色名称查询" v-model="name" />
					</view>
					<view class="cu-form-group">
						<view class="title">备注：</view>
						<view>
							<input prefix-icon="search" placeholder="请输入备注查询" v-model="description" />
						</view>
					</view>
		...
		<view class="stock-list" style="[{animation: 'show 0.5s 1'}]">
			<mescroll-uni ref="mescrollRef" @init="mescrollInit" :top="top" @down="downCallback" @up="upCallback">
				<view
					...
					style="width: 100vw;" v-for="(role,index) in roles" :key="index"
					@tap="gorolesDetail(role)" :class="modalName=='move-box-'+ index?'move-cur':''"
					:data-target="'move-box-' + index">
					<view class="stock-item" style="min-height: 60px;width: 100vw">
						<view class="item-row">
							<text class="label">名称：</text>
							<text class="value name">{{ role.name || '-' }}</text>
							<view style="margin-right: 20rpx; font-weight: bold;">
								<text v-if="role.enabled == '0'" style="color:red;">禁用</text>
								<text v-if="role.enabled == '1'" style="color:green;">启用</text>
							</view>
						</view>
						<view class="item-row">
							<text class="label">编号：</text>
							<text class="value">{{ role.type || '-' }}</text>
						</view>
						<view class="item-row">
							<text class="label">价格屏蔽：</text>
							<text  style="font-size: 8px; width: 70%;">{{ role.priceLimitStr || '-' }}</text>
						</view>
						<view class="item-row">
							<text class="label">备注：</text>
							<text class="value">{{ role.description || '-' }}</text>
						</view>
					</view>
				</view>
			</mescroll-uni>
		...
		methods: {
			...
			loadinfo() {
				this.$http.get(this.rolesUrl, {
					params: {
						search: '{"name":"' + this.name + '","description":"' + this.description + '"}',
					}
				}).then(res => {
			...
			upCallback(page) {
				...
				params.search = '{"name":"' + this.name + '","description":"' + this.description +'"}',
					//联网加载数据
					this.$http.get(this.rolesUrl, {
						params: params
					}).then(res => {
						if (page.num == 1) {
							this.roles = []; //如果是第一页需手动制空列表
						}
						if (res.data.code == "200") {
							this.roles = this.roles.concat(res.data.data.rows); //追加新数据
			...
			gorolesDetail(role) {
				...
				uni.navigateTo({
					url: '/pages/material/role/rolesInfo?roles=' + encodeURIComponent(JSON.stringify(
						role)) + '&action=edit'
				});
			},
			fabClick() {
				uni.navigateTo({
					url: '/pages/material/roles/rolesInfo?action=add'
				});
			},
			ListTouchStart(e) {
