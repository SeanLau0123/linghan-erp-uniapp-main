	...
	<view> <cu-custom :bgColor="NavBarColor" :isBack="true" backRouterName="material">
		...
		<view class="search-box">
			<view class="input-row">
				<input class="search-input" placeholder="请输入登录名称查询" v-model="name" @confirm="confirm" />
				<uni-icons type="scan" size="25" @tap="showScan"></uni-icons>
				<uni-icons type="search" @tap="openuserSearch" size="25" style='margin-right: 5px;'></uni-icons>
			</view>
		</view>
			...
			<view style="height: 800px; width: 100%;float: right;">
				<form style="height: 800px;">
					<view class="cu-form-group" style="margin-top:60px;">
						<view class="title">登录名称：</view>
						<input prefix-icon="search" placeholder="请输入登录名称查询" v-model="name" />
					</view>
					<view class="cu-form-group">
						<view class="title">用户姓名：</view>
						<view>
							<input prefix-icon="search" placeholder="请输入用户姓名查询" v-model="userName" />
						</view>
					</view>
		...
		<view class="stock-list" style="[{animation: 'show 0.5s 1'}]">
			<mescroll-uni ref="mescrollRef" @init="mescrollInit" :top="top" @down="downCallback" @up="upCallback">
				<view
					...
					style="width: 100vw;" v-for="(user,index) in users" :key="index"
					@tap="gousersDetail(user)" :class="modalName=='move-box-'+ index?'move-cur':''"
					:data-target="'move-box-' + index">
					<view class="stock-item" style="min-height: 60px;width: 100vw">
						<view class="item-row">
							<text class="label">登录名称：</text>
							<text class="value name">{{ user.loginName || '-' }}</text>
						...
						<view class="item-row">
							...
							<text class="value">{{ user.userType || '-' }}</text>
						</view>
						<view class="item-row">
							<text class="label">电话：</text>
							<text class="value">{{ user.phonenum || '-' }}</text>
						</view>
						<view class="item-row">
							<text class="label">备注：</text>
							<text class="value">{{ user.description || '-' }}</text>
						</view>
					</view>
				</view>
			</mescroll-uni>
		...
		methods: {
			...
			loadinfo() {
				this.$http.get(this.usersUrl, {
					params: {
						search: '{"loginName":"' + this.name + '","userName":"' + this.userName + '"}',
					}
				}).then(res => {
			...
			upCallback(page) {
				...
				params.search = '{"loginName":"' + this.name + '","userName":"' + this.userName +'"}',
					//联网加载数据
					this.$http.get(this.usersUrl, {
						params: params
					}).then(res => {
						if (page.num == 1) {
							this.users = []; //如果是第一页需手动制空列表
						}
						if (res.data.code == "200") {
							this.users = this.users.concat(res.data.data.rows); //追加新数据
			...
			gousersDetail(user) {
				...
				uni.navigateTo({
					url: '/pages/material/user/usersInfo?users=' + encodeURIComponent(JSON.stringify(
						user)) + '&action=edit'
				});
			},
			fabClick() {
				uni.navigateTo({
					url: '/pages/material/users/usersInfo?action=add'
				});
			},
			ListTouchStart(e) {
