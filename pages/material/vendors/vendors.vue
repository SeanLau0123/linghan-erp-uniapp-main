...
<template>
	<view> <cu-custom :bgColor="NavBarColor" :isBack="true" backRouterName="material">
			...
			<block slot="content">供应商信息</block>
		</cu-custom>
		<view class="search-box">
			<view class="input-row">
				...
				<uni-icons type="search" @tap="openvendorSearch" size="25" style='margin-right: 5px;'></uni-icons>
			</view>
		</view>
		...
		<view class="stock-list" style="[{animation: 'show 0.5s 1'}]">
			<mescroll-uni ref="mescrollRef" @init="mescrollInit" :top="top" @down="downCallback" @up="upCallback">
				<view
					...
					style="width: 100vw;" v-for="(vendor,index) in vendors" :key="index"
					@tap="govendorsDetail(vendor)" :class="modalName=='move-box-'+ index?'move-cur':''" :data-target="'move-box-' + index">
					<view class="suppliers-item" style="min-height: 100px;width: 100vw">
						<view class="suppliers-row">
							<text class="label">名称：</text>
							<text class="value name">{{ vendor.supplier || '-' }}</text>
							<view style="margin-right: 20rpx; font-weight: bold;">
								<text v-if="vendor.enabled == '0'" style="color:red;">禁用</text>
								<text v-if="vendor.enabled == '1'" style="color:green;">启用</text>
							</view>
						</view>
						<view class="suppliers-row">
							<text class="label">联系人：</text>
							<text class="value">{{ vendor.contacts || '-' }}</text>
						</view>
						<view class="suppliers-row">
							<text class="label">手机号码：</text>
							<text class="value">{{vendor.telephone || '-' }}</text>
							<text class="label">联系电话：</text>
							<text class="value">{{ vendor.phoneNum}}</text>
						</view>
						<view class="suppliers-row">
							<text class="label">电子邮箱：</text>
							<text class="value">{{ vendor.email || '-' }}</text>
						</view>
						<view class="suppliers-row">
							<text class="label">备注：</text>
							<text class="value">{{ vendor.description || '-' }}</text>
						</view>
					</view>
				</view>
			</mescroll-uni>
		...
		methods: {
			...
			showScan() {
				...
				uni.scanCode({
					success: function(res) {
						that.vendorsInfo = res.result
					}
				});
			},
			govendorsDetail(vendor) {
				this.mescroll.resetUpScroll()
				uni.navigateTo({
					url: '/pages/material/vendors/vendorsInfo?vendors=' + encodeURIComponent(JSON.stringify(
						vendor)) + '&action=edit'
				});
			},
			fabClick(){
				uni.navigateTo({ url: '/pages/material/vendors/vendorsInfo?action=add'
				});
			},
			loadinfo() {
				this.$http.get(this.vendorsUrl, {
					params: {
			...
			handleSearch() {
				...
				this.filteredvendors = this.vendors.filter(vendor =>
					(vendor.name && vendor.name.toLowerCase().includes(keyword)));
			},
			resetSearch() {
				this.name = '';
				this.telephone = '';
			...
			upCallback(page) {
				...
				params.search = JSON.stringify({
					...
					this.$http.get(this.vendorsUrl, {
						...
						if (page.num == 1) {
							this.vendors = []; //如果是第一页需手动制空列表
						}
						if (res.data.code == "200") {
							this.vendors = this.vendors.concat(res.data.data.rows); //追加新数据
