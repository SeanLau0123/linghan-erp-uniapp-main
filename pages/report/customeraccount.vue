	...
	export default {
		...
		methods: {
			...
			loadinfo() {
				...
				if(new Date(this.beginTime) > new Date(this.endTime) || new Date(this.endTime) < new Date(this.beginTime))
				{
					this.$tip.alert('时间范围选择错误！');
					return;
				}
				if (this.debType) {
