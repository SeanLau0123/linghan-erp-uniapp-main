		...
		<scroll-view>
			<view class="action">
				<view class="search-box">
					...
					<text class="stock-info">总库存：{{currentStock}}　　　　总库存余额：{{currentStockPrice.toFixed(2)}}
						总重量：{{currentWeight || '0'}}
					</text>
				</view>
			...
			<view class="stock-list" :style="[{animation: 'show 0.5s 1'}]">
				<view class="stock-item" v-for="stock in stocks" :key="stock.name">
					<view class="item-row">
						...
						<text class="value name">{{ stock.name || '-'}}({{stock.mBarCode}})</text>
					</view>
					<view class="item-row">
					...
					<view class="item-row">
						...
						<text class="value">{{stock.currentStock || '-' }}</text>
						<text class="label">库存金额：</text>
						<text class="value">¥{{stock.currentStockPrice.toFixed(2) || '-' }}</text>
					</view>
				</view>
			</view>
		</scroll-view>
			...
			loadinfo() {
				...
				this.$http.get(this.stockUrl, {
					...
					if (res.data.code == "200") {
						...
						this.currentStockPrice = result.currentStockPrice;
						this.currentWeight = Number(result.currentWeight) || 0;
