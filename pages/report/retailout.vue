		...
		<scroll-view>
			<view class="action">
				<view class="search-box">
					...
					<text style="text-align:center; font-weight: bold">实际零售金额：¥{{realityPriceTotal.toFixed(2)}}</text>
				</view>
			...
			<view class="stock-list" :style="[{animation: 'show 0.5s 1'}]">
				<view class="stock-item" v-for="item in retailout" :key="item.materialName">
					<view class="item-row">
						<text class="label">名称：</text>
						<text class="value name">{{ item.materialName || '-' }}({{item.barCode}})</text>
					...
					<view class="item-row">
						...
						<text class="value">{{item.outSum || '0' }}</text>
						<text class="label">零售金额：</text>
						<text class="value">¥{{item.outSumPrice.toFixed(2) || '-' }}</text>
					</view>
					<view class="item-row">
						<text class="label">退货数量：</text>
						<text class="value">{{item.inSum || '0' }}</text>
						<text class="label">退货金额：</text>
						<text class="value">¥{{item.inSumPrice.toFixed(2) || '-' }}</text>
					</view>
					<view class="item-row">
						<text class="label">实际零售金额：</text>
						<text class="value">¥{{item.outInSumPrice.toFixed(2) || '-' }}</text>
					</view>
				</view>
			</view>
		</scroll-view>
	...
	export default {
		...
		data() {
			return {
				...
				retailout: [],
				realityPriceTotal: 0,
				retailoutUrl: '/depotItem/retailOut',
		...
		methods: {
			...
			getSupplierData() {
				let that = this
				this.$http.post("/supplier/findBySelect_retail", {
				}).then(res => {
			...
			loadinfo() {
				...
				this.$http.get(this.retailoutUrl, {
					params: params
				}).then(res => {
					if (res.data.code == "200") {
						let result = res.data.data;
						this.realityPriceTotal = result.realityPriceTotal;
						this.retailout = result.rows;
					}
				}).catch(e => {
					console.log("请求错误", e)
				})
